import { IQueue, TQueueAddItemResponse, TQueueItem, E_QUEUE_PACKAGE_STATUS, TQueueMapKeysAndEvents } from "../../../interfaces/application/queue/IQueue.js"
...
export class Queue implements IQueue {
  ...
  public addItem(keyGroup: string, eventId: string, data: any, customLimitPackage: number | undefined): TQueueAddItemResponse {
    ...
    if (!this.queue.has(keyGroup)) {
      ...
      const firstPackage = { lastEventCreatedAt: currentTs, status: E_QUEUE_PACKAGE_STATUS.STACKING,  limitPackageSize, events: new Map().set(eventId, data) }
    ...
    group!.package.set(newLastPackageId, { lastEventCreatedAt: currentTs, limitPackageSize, status: E_QUEUE_PACKAGE_STATUS.STACKING, events: new Map().set(eventId, data) })
  ...
  public getPackagesWithTimeLimitExpired(): Array<{keyGroup: string, packageIndex: number}> {
    ...
    for (const [keyGroup, valueGroup] of this.queue){
      for (const [packageIndex, packageItem] of valueGroup.package) {
        if (packageItem.status === E_QUEUE_PACKAGE_STATUS.PROCESSING) continue
  ...
  public getAndUpdateStatusPackagesToProcessing(keyGroup: string, packageIndex: number): TQueueMapKeysAndEvents | void{
    ...
    if(packageGroup!.status === E_QUEUE_PACKAGE_STATUS.PROCESSING) return
    packageGroup!.status = E_QUEUE_PACKAGE_STATUS.PROCESSING
